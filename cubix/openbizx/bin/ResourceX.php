...
class Resource
{
    ...
    {
        // use class map first
        if (isset( self::$_coreClassMap[$className]  )) {
            $classFile = OPENBIZ_HOME . self::$_coreClassMap[$className];
    ...
    public static function getCoreLibFilePath($className) {
        return self::_getCoreLibFilePath($className);
    }
    ...
    /**
     * Check is class name core class?
     *      
    ...
    {
        return isset(self::$_coreClassMap[$className]);
    }    
     ...
     * @var array
     */
    private static $_coreClassMap = array(
