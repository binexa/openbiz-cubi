...
class ModuleLoader
{
    ...
    {
        return Openbiz::$app->getDbConnection($this->dbName);
    }
    ...
    public function loadModule($installSql = false)
    {
        ...
        $modfile = Openbiz::$app->getModulePath() . "/$module/mod.xml";
        ...
        Openbiz::getObject("system.do.ModuleCachedDO")->cleanCache();
        return true;
    }
    ...
    public function upgradeModule($forceUpgrade = false)
    {
        ...
        $modFolder = Openbiz::$app->getModulePath() . "/" . $this->objectName;
    ...
    static public function isModuleOld($module, $dbName = null)
    {
        ...
        $modFolder = Openbiz::$app->getModulePath() . "/" . $module;
        $modfile = $modFolder . "/mod.xml";
        $xml = simplexml_load_file($modfile);
        $mod_ver = $xml['Version'];
    ...
    protected function upgradeSQLs($baseVersion, $targetVersion)
    {
        ...
        $upgradeFile = Openbiz::$app->getModulePath() . "/" . $this->objectName . "/upgrade.xml";
    ...
    {
        $this->log("Install Module Sql.");
        $sqlfile = Openbiz::$app->getModulePath() . "/" . $this->objectName . "/mod.install.sql";
        if (!file_exists($sqlfile)) {
            return true;
        }
    ...
    protected function installModule($forceInstall = true)
    {
        ...
        if (!$skipDBChanges) {
            $this->installEventObservers($xml);
        }
        ...
        if ($version) {//if installed, then upgrade, if not installed just fresh install, dont process upgradesql
            if (!$skipDBChanges) {
                $this->upgradeSQLs($version, $modVersion);
            }
        }
    ...
    protected function installResource($xml)
    {
        ...
        if (isset($xml->Files) && isset($xml->Files->Copy)) {
            foreach ($xml->Files->Copy as $copy) {
                // echo "Copy ".Openbiz::$app->getModulePath().'/'.$this->objectName.'/'.$copy['From'].' > '.OPENBIZ_APP_PATH.'/'.$copy['ToDir'].PHP_EOL;
                $toDirs = glob(OPENBIZ_APP_PATH . '/' . $copy['ToDir']);
                //print_r($toDirs);
                $fromFiles = glob(Openbiz::$app->getModulePath() . '/' . $this->objectName . '/' . $copy['From']);
                //print_r($fromFiles);
    ...
    protected function loadMenuItem($menuItem, $parentMenuName = '')
    {
        ...
        foreach ($menuItem->MenuItem as $m) {
            if ($this->loadMenuItem($m, $name) == false) {
                return false;
            }
        }
        return true;
    ...
    protected function installWidgets($xml)
    {
        ...
        if (isset($xml->Widgets) && isset($xml->Widgets->Widget)) {
            ...
            //clean  obj cache
            $menuObj = Openbiz::getObject("system.do.WidgetDO");
            $menuObj->CleanCache();

            foreach ($xml->Widgets->Widget as $m) {
                if ($this->loadWidget($m) == false) {
                    return false;
                }
            }
        }
    ...
    protected function loadWidget($widget, $moduleName = '')
    {
        ...
        $configable = Openbiz::getObject($name)->configable;
        $configable = (int) $configable;

        $do = Openbiz::getObject("system.do.WidgetDO");
        $recArr = array(
            "name" => $name,
            "module" => $module,
            "title" => $title,
    ...
    protected function installEventObservers($xml)
    {
        ...
        if (isset($xml->EventObservers) && isset($xml->EventObservers->Observer)) {
            ...
            //clean  obj cache
            $obsObj = Openbiz::getObject("eventmgr.do.EventObserverDO");
            $obsObj->CleanCache();

            for ($i = 0; $i < count($xml->EventObservers->Observer); $i++) {
                $m = $xml->EventObservers->Observer[$i];
                if ($this->loadObserver($m) == false) {
                    return false;
                }
            }
        }
    ...
    protected function loadObserver($observer, $moduleName = '')
    {
        ...
        $do = Openbiz::getObject("eventmgr.do.EventObserverDO");
        $recArr = array(
            "name" => $name,
    ...
    protected function installMetaDo()
    {
        ...
        foreach ($g_MetaFiles as $metaFile) {
            $metaName = str_replace('/', '.', str_replace(array(Openbiz::$app->getModulePath() . '/', '.xml'), '', $metaFile));
            // load do
