    ...
    static public function parse($data)
    {
        ...
        while (!(self::$finished && $i >= $len) && !$error && !$timeout_passed) {
            ...
            while ($i < $len) {
                ...
                if ((($i == ($len - 1) && ($ch == '-' || $ch == '/')) || ($i == ($len - 2) && (($ch == '-' && $buffer[$i + 1] == '-') || ($ch == '/' && $buffer[$i + 1] == '*')))) && !self::$finished) {
                    break;
                }

                // Comments
                if ($ch == '#' || ($i < ($len - 1) && $ch == '-' && $buffer[$i + 1] == '-' && (($i < ($len - 2) && $buffer[$i + 2] <= ' ') || ($i == ($len - 1) && self::$finished))) || ($i < ($len - 1) && $ch == '/' && $buffer[$i + 1] == '*')
                ) {
