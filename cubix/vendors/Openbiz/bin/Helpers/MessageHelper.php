...
class MessageHelper
{
    ...
    public static function loadMessage($messageFile, $packageName = "")
    {
        ...
        // 2. Openbiz::$app->getModulePath() . "/$moduleName/message/" . $messageFile;
        ...
        } else if (is_file(Openbiz::$app->getModulePath() . "/" . $messageFile)) {
            return parse_ini_file(Openbiz::$app->getModulePath() . "/" . $messageFile);
        } else {
            if (isset($packageName) && $packageName != "") {
                $dirs = explode('.', $packageName);
                $moduleName = $dirs[0];
                $msgFile = Openbiz::$app->getModulePath() . "/$moduleName/message/" . $messageFile;
                if (is_file($msgFile)) {
