...
class BizDataObj_Assoc
{
    ...
    public static function addRecord($dataObj, $recArr, &$isParentObjUpdated)
    {
        if ($dataObj->association["Relationship"] == "M-M") {
            ...
            throw new Openbiz\Data\Exception("You cannot add a record in dataobj who doesn't have M-M or M-1 or Self-Self relationship with its parent object");
            return false;
        }
    }
    ...
    private static function _addRecordMtoM($dataObj, $recArr)
    {
        ...
        if ($xDataObj) {   // get new record from XDataObj
            $xObj = Openbiz::getObject($xDataObj);
            ...
            if ($xObj->mainTableName != $dataObj->association["XTable"]) {
                throw new Openbiz\Data\Exception("Unable to create a record in intersection table: XDataObj's main table is not same as XTable.");
                //return false;
            }
            ...
            if ($ok === false) {
                throw new Openbiz\Data\Exception($xObj->getErrorMessage());
                //return false;
            }
        } else {
            ...
            try {
                Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Associate Insert Sql = $sql");
                $db->query($sql);
            } catch (Exception $e) {
                Openbiz::$app->getLog()->log(LOG_ERR, "DATAOBJ", "Query Error: " . $e->getMessage());
                throw new Openbiz\Data\Exception("Query Error: " . $e->getMessage());
                return false;
            }
        }
    ...
    private static function _addRecordMto1($dataObj, $recArr)
    {
        ...
        $asscObj = Openbiz::getObject($dataObj->association["AsscObjName"]);
    ...
    private static function _addRecord1toM($dataObj, $recArr)
    {
        ...
        $newDO = Openbiz::getObject($dataObj->objectName, 1);
    ...
    private static function _removeRecordMtoM($dataObj, $recArr)
    {
        ...
        try {
            Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Associate Delete Sql = $sql");
            $db->query($sql);
        } catch (Exception $e) {
            Openbiz::$app->getLog()->log(LOG_ERR, "DATAOBJ", "Query Error: " . $e->getMessage());
            throw new Openbiz\Data\Exception("Query Error: " . $e->getMessage());
