        ...
        try {
            if ($this->cacheLifeTime > 0) {
                ...
                if ($cacheSvc->test($cache_id)) {
                    //Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Cache Hit. Query Sql = ".$sql);
                    $resultSetArray = $cacheSvc->load($cache_id);
                } else {
                    Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Query Sql = " . $sql);
    ...
    private function _getNumberRecords($db, $sql)
    {
        ...
        try {
            if ($this->cacheLifeTime > 0) {
                ...
                //try to process cache service.
                $cacheSvc = Openbiz::getService(CACHE_SERVICE);
                $cacheSvc->init($this->objectName, $this->cacheLifeTime);
                if ($cacheSvc->test($cache_id)) {
                    //Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", ". Query Sql = ".$rewritesql);
