...
class BizDataObj extends BizDataObj_Lite
{
     ...
     *
     * @return boolean
     * @todo: throw Openbiz\Data\Exception
     * */
    public function validateInput()
    {
        ...
        if (count($this->errorFields) > 0) {
            ...
            throw new Openbiz\Validation\Exception($this->errorFields);
            //return false;
        }
    ...
    public function updateRecord($recArr, $oldRecord = null)
    {
        ...
        if ($sql) {
            ...
            try {
                ...
                Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Update Sql = $sql");
                $db->query($sql);
                ...
                if ($e instanceof Openbiz\Data\Exception)
                    throw $e;
                else {
    ...
    private function _postUpdateLobFields(&$recArr)
    {
        ...
        foreach ($this->bizRecord as $field) {
            if (isset($recArr[$field->objectName]) && $field->isLobField() && $field->column != "") {
                ...
                Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Update lob Sql = $sql");
                $stmt = $db->prepare($sql);
                ...
                try {
                    ...
                    $this->errorMessage = $this->getMessage("DATA_ERROR_QUERY") . ": " . $sql . ". " . $e->getMessage();
                    Openbiz::$app->getLog()->log(LOG_ERR, "DATAOBJ", "Update lob error = $sql");
                    fclose($fp);
                    throw new Openbiz\Data\Exception($this->errorMessage);
                    return null;
                }
    ...
    protected function generateId($isBeforeInsert = true, $tableName = null, $idCloumnName = null)
    {
        ...
        $genIdService = Openbiz::getService(GENID_SERVICE);
        if ($this->m_db) {
            $db = $this->m_db;
        } else {
            $db = $this->getDBConnection("READ");
        }
        $dbInfo = Openbiz::$app->getConfiguration()->getDatabaseInfo($this->databaseAliasName);
    ...
    public function insertRecord($recArr)
    {
        ...
        try {
            ...
            if ($sql) {
                Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Insert Sql = $sql");
            ...
            Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "New record Id is " . $recArr["Id"]);
        } catch (Exception $e) {
            Openbiz::$app->getLog()->log(LOG_ERR, "DATAOBJ", "Query Error : " . $e->getMessage());
            $this->errorMessage = $this->getMessage("DATA_ERROR_QUERY") . ": " . $sql . ". " . $e->getMessage();
            throw new Openbiz\Data\Exception($this->errorMessage);
            ...
            if ($cascadeType == 'Delete') {
                if ($objRef->onDelete == "Cascade") {
                    ...
                    if (count($refObj->directFetch($sql, 1)) == 1) {
                        throw new Openbiz\Data\Exception($this->getMessage("DATA_UNABLE_DEL_REC_CASCADE", array($objRef->objectName)));
                    }
                    return;
                }
                ...
                if ($objRef->onUpdate == "Cascade") {
                    ...
                    // check if objRef has records
                    $refObj = Openbiz::getObject($objRef->objectName);
                    $sql = "[" . $objRef->fieldRef . "]='" . $refField->oldValue . "'";
                    if ($column2 && $fieldVal2) {
                        $sql .= " AND " . $column2 . "='" . $fieldVal2 . "'";
                    }
                    if (count($refObj->directFetch($sql, 1)) == 1) {
                        throw new Openbiz\Data\Exception($this->getMessage("DATA_UNABLE_UPD_REC_CASCADE", array($objRef->objectName)));
                    }
                    return;
                }
            }
            try {
                Openbiz::$app->getLog()->log(LOG_DEBUG, "DATAOBJ", "Cascade $cascadeType Sql = $sql");
                $db->query($sql);
            } catch (Exception $e) {
                Openbiz::$app->getLog()->log(LOG_Err, "DATAOBJ", "Cascade $cascadeType Error: " . $e->getMessage());
                $this->errorMessage = $this->getMessage("DATA_ERROR_QUERY") . ": " . $sql . ". " . $e->getMessage();
                throw new Openbiz\Data\Exception($this->errorMessage);
            }
        }
